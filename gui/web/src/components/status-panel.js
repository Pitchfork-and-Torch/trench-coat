...
export function applyStatus(s) {
  ...
  else if (s.owner_alive || s.pending) label = "ENGAGING";

  if (pill) {
    pill.textContent = label;
    pill.classList.toggle("on", label === "CLOAKED");
    pill.classList.toggle("hold", label === "HOLD");
    pill.classList.toggle("engaging", label === "ENGAGING");
  }
  ...
  renderHops(s.running ? s.snapshot?.hops || [] : []);
}
...
export function renderHops(hops = []) {
  ...
  const healthClass = { healthy: "healthy", degraded: "degraded", dead: "dead", unknown: "unknown" };
  hops.forEach((h) => {
    const li = document.createElement("li");
    const health = healthClass[h.health] || "unknown";
    li.innerHTML = `
      <span class="dot ${health}"></span>
      <span>${escapeHtml(h.id || "?")}</span>
      <span>${Number.isFinite(Number(h.latency_ms)) ? Math.round(Number(h.latency_ms)) + " ms" : " - "}</span>
    `;
    ul.appendChild(li);
  });
}
