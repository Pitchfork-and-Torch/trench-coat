...
export function createCityMap(canvas) {
  ...
  function layoutFromStatus(status) {
    const live = status?.snapshot?.hops || [];
    const draft = (status?.draft_hops || []).filter((h) => h.enabled !== false);
    const hops =
      status?.running && live.length
        ? live
        : draft.map((hop) => ({
            id: hop.label || hop.id,
            health: "draft",
    ...
    const margin = 90;
    // Never invent healthy hops from proxy_chain length or a running flag.
    const mid = hops.map((hop, i) => {
  ...
  function drawBuilding(x, y, label, sub, hot, dead) {
    ...
    ctx.fillStyle = dead ? "#2a0810" : hot ? "rgba(107, 143, 122, 0.18)" : "#12141f";
    ctx.strokeStyle = dead ? "#c45c6a" : hot ? "#6b8f7a" : "rgba(201, 162, 39, 0.4)";
    ctx.lineWidth = dead ? 2 : 1.5;
    ctx.beginPath();
    ctx.rect(x - bw / 2, y - bh, bw, bh);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = dead ? "rgba(196, 92, 106, 0.55)" : hot ? "rgba(107, 143, 122, 0.7)" : "rgba(201, 162, 39, 0.28)";
    for (let r = 0; r < 3; r++) {
      for (let c = 0; c < 2; c++) {
    ...
    if (sub) {
      ctx.fillStyle = dead ? "#c45c6a" : "#6b8f7a";
  ...
  function draw(status) {
    ...
    const live = lastStatus?.snapshot?.hops || [];
    const draft = (lastStatus?.draft_hops || []).filter((h) => h.enabled !== false);
    const n = lastStatus?.running && live.length ? live.length : draft.length;
    const expected = n + 2;
    ...
    ctx.strokeStyle = tripped ? "rgba(196, 92, 106, 0.12)" : "rgba(107, 143, 122, 0.14)";
    ctx.lineWidth = 1;
    ...
    // fabric
    ctx.strokeStyle = tripped ? "rgba(196, 92, 106, 0.55)" : "rgba(201, 162, 39, 0.35)";
    ctx.lineWidth = 2;
    ctx.beginPath();
  ...
  return {
    setStatus(s) {
      const draft = lastStatus?.draft_hops;
      lastStatus = { ...(s || {}), draft_hops: s?.draft_hops || draft || [] };
      layoutFromStatus(lastStatus);
    },
    setDraftHops(hops) {
      lastStatus = { ...lastStatus, draft_hops: hops || [] };
      layoutFromStatus(lastStatus);
    },
  };
}
