...
import { createCircuitDesk } from "./components/circuit-desk.js";
...
async function wireActions() {
  ...
  $("btn-engage")?.addEventListener("click", async () => {
    try {
      if (desk) await desk.persist();
      await postJSON("/api/legal/accept", {});
      const chain = desk?.getChain()?.name;
      const r = await postJSON("/api/cloak/up", {
        accept_legal: true,
        wait_tor: 30,
        chain: chain || undefined,
      });
      toast(r.message || (r.running ? `Cloak engaged pid=${r.pid}` : `Engage pid=${r.pid} (not cloaked yet)`));
    } catch (e) {
      toast(String(e.message || e));
    }
  });
  ...
  $("btn-disengage")?.addEventListener("click", async () => {
    try {
      const r = await postJSON("/api/cloak/down", {});
      toast(r.message || "Disengaged.");
    } catch (e) {
      toast(String(e.message || e));
    }
  });
...
const desk = createCircuitDesk({
  toast,
  onDraft(hops) {
    if (map) map.setDraftHops(hops);
  },
});
loadPresets((name) => {
  toast(`Profile ${name} loaded onto the desk.`);
  desk.reload();
});
loadDossiers();
wireActions();
