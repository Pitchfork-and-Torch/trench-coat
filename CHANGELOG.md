...
## 1.3.1 - Status Honesty

Pid fingerprint on Disengage. Hop probe CONNECT-refused is DEGRADED, not HEALTHY. Status must not paint CLOAKED from a live port or a stale pid file.

### Honesty

- `cloak.pid` stores create_time. Disengage kills only when the live process matches that fingerprint. Reused PIDs and legacy bare pid files are stale (lock cleared, process not killed).
- Windows CTRL_BREAK only for breakaway Engage children. Stop file first so `trench up` can engine.stop.
- SOCKS5/HTTP probe: greeting/listener up but CONNECT to the probe dest refused -> DEGRADED. Closed port stays DEAD. DEGRADED hops remain chainable.
- `/api/status` probe fallback never sets `running` from port-open. Stale status bus with a dead owner is not CLOAKED. Engage may return `pending` (process live, not cloaked yet) or 503 if the child exits.
- Nexus: ENGAGING pill while owner is alive and entry is not up. City map does not invent healthy hops from `proxy_chain` length.

### Responsibility

Legal-first only. Not a crime toolkit. Fail-closed and `--accept-legal` unchanged. AGPL-3.0-or-later.
