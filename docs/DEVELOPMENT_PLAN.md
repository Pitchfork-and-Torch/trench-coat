...
## Phase 7.3.1 - Status Honesty -> **v1.3.1**

- [x] Pid create_time fingerprint; Disengage refuses PID reuse kills
- [x] Hop probe CONNECT-refused is DEGRADED (listener up), not HEALTHY or DEAD
- [x] Status probe fallback and stale bus do not paint CLOAKED
- [x] **Shipped as v1.3.1** (Status Honesty)

## Phase 7.3 - Hop Receipt -> **v1.3.0**

- [x] `trench receipt --json` prints hop types and enabled flags (no secrets)
- [x] **Shipped as v1.3.0** (Hop Receipt)

## Phase 7.2 - Circuit Desk -> **v1.2.0**

- [x] Command Nexus hop workshop (enable, reorder, drop, templates)
- [x] `/api/chain` GET/PUT + `/api/templates` (no home paths in JSON)
...
- [x] Brand system + landing rebuild (watercolor noir)  
- [x] Command Nexus theme tokens + Velvet marketing Nexus shot  
- [x] How-the-cloak-works infographic (exact labels)  
- [x] Residual doctor hard-KS + Windows Tor egress guidance  
- [x] pipx-first install docs  
- [x] **Shipped as v1.1.0** (Velvet Collar)
...
- [x] Windows hard KS Tor egress allow-list polish (1.1)  
- [x] pipx / published wheel first-class docs path (1.1 docs; PyPI wheel later)  
