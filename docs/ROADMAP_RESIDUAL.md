...
*What is still open after Status Honesty **v1.3.1**. Honest scope for operators and auditors.*
...
| Item | Priority | Notes |
|------|----------|-------|
| Richer chain builder UI | **Done in 1.2 Circuit Desk** | Templates + reorder + enable/drop |
| Pid-owned Engage / Disengage | **Done in 1.2; fingerprint in 1.3.1** | Stop file + create_time fingerprint; Windows CTRL_BREAK only for breakaway children; Nexus waits for live cloak or honest pending/fail. Do not kill reused PIDs. |
...
## Done in 1.3.1 Status Honesty

- Pid create_time fingerprint on Disengage; reused PIDs are not killed
- SOCKS5/HTTP CONNECT-refused probes report DEGRADED, not HEALTHY
- `/api/status` does not claim CLOAKED from a listening port or a stale bus
- Public version stamps (package, CLI `--version`, Nexus, landing/AEO) set to 1.3.1
