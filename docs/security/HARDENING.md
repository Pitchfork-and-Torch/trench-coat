...
### Residual hard rules (doctor)

- `trench doctor` includes **`hard_ks_residual`**: warns if leftover `TrenchCoat-*` Windows rules, Linux `inet trenchcoat_hard`, or non-empty macOS `pf` anchor remain.  
- Remediation: `trench killswitch disarm` as admin/root, then re-run doctor.

### Windows hard KS + Tor egress allow-list

Hard mode blocks outbound after loopback allows. Tor must still reach the public network.

1. Prefer soft fail-closed for daily use.  
2. Run `trench killswitch hard --dry-run` first.  
3. Doctor check **`windows_tor_egress_allowlist`** reports detected Tor binaries.  
4. Typical Tor Browser path: `%LOCALAPPDATA%\\Tor Browser\\Browser\\TorBrowser\\Tor\\tor.exe`  
5. On apply, Trench Coat attempts `TrenchCoat-Allow-Tor-*` program allows for detected paths.  
6. Keep the undo script. Signed WFP is not shipped.
