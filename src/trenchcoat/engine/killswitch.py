...
class KillSwitch:
    ...
    def _windows_hard_plan(self, allow_ports: list[int]) -> dict:
        def apply() -> bool:
            ...
            for i, tor_path in enumerate(self.windows_tor_program_candidates()):
                name = f"{RULE_PREFIX}-Allow-Tor-{i}"
                self._run(
                ...
                if tr.returncode == 0:
                    self.state.messages.append(f"Allowed Tor egress program: {tor_path}")
                else:
                    self.state.messages.append(
                        f"Could not add Tor program allow for {tor_path}."
                    )
            return True

        tor_bins = self.windows_tor_program_candidates()
        tor_note = (
            "Detected Tor program path(s) for allow-list: "
            + ", ".join(str(p) for p in tor_bins)
            if tor_bins
            else (
                "No Tor.exe auto-detected. Before hard apply, allow your Tor / Tor Browser "
                "binary outbound. Typical: "
        ...
        return {
            "describe": [
                ...
                "See docs/security/HARDENING.md section Windows hard KS + Tor egress allow-list.",
            ],
            "apply": apply,
        }
...
""",
    ...
    def detect_residual_rules(self) -> dict[str, object]:
        """Detect leftover hard kill-switch rules after disarm (doctor residual check)."""
        system = platform.system()
        found: list[str] = []
        detail = ""
        ...
        if system == "Windows":
            ...
            if result.returncode != 0:
                return {
                    ...
                    "residual": False,
                    "rules": [],
                    "detail": "Could not enumerate firewall rules (permissions?).",
                    "ok_to_probe": False,
                }
            for line in result.stdout.splitlines():
                if "Rule Name:" in line and RULE_PREFIX in line:
            ...
            detail = (
                f"Found {len(found)} residual {RULE_PREFIX}-* Windows firewall rule(s)."
                if found
                else f"No residual {RULE_PREFIX}-* Windows firewall rules."
            )
        elif system == "Linux":
            result = self._run(["nft", "list", "table", "inet", "trenchcoat_hard"])
            if result.returncode == 0 and result.stdout.strip():
                found.append("inet trenchcoat_hard")
                detail = "nftables table inet trenchcoat_hard still present."
            else:
                detail = "No nftables table inet trenchcoat_hard."
        elif system == "Darwin":
            result = self._run(["pfctl", "-a", "trenchcoat", "-sr"])
            out = (result.stdout or "") + (result.stderr or "")
            if result.returncode == 0 and out.strip() and "No ALTQ" not in out:
                if any(tok in out.lower() for tok in ("block", "pass", "from", "to")):
                    found.append("pf anchor trenchcoat")
                    detail = "pf anchor trenchcoat appears non-empty."
                else:
                    detail = "pf anchor trenchcoat empty or inactive."
            else:
                detail = "pf anchor trenchcoat empty or not loaded."
        else:
            detail = f"Residual rule probe not implemented for {system}."
        ...
        return {
            ...
            "ok_to_probe": True,
            "remediation": (
                "Run `trench killswitch disarm` (admin/root), then re-run `trench doctor`."
                if found
                else ""
            ),
        }
    ...
    @staticmethod
    def windows_tor_program_candidates() -> list[Path]:
        """Common Windows Tor / Tor Browser binary paths for hard-KS allow-list docs."""
        home = Path.home()
        ...
        candidates = [
            ...
            prog / "Tor" / "tor.exe",
            prog / "Tor Browser" / "Browser" / "TorBrowser" / "Tor" / "tor.exe",
