...
async def run_self_test(
    ...
    *,
    ...
    if hard_ks_dry_run:
        ...
        residual = ks.detect_residual_rules()
        if residual.get("residual"):
            checks.append(
                CheckResult(
                    id="hard_ks_residual",
                    title="Hard kill-switch residual rules",
                    status="warn",
                    detail=(
                        f"{residual.get('detail', 'Residual hard KS rules present.')} "
                        f"{residual.get('remediation', '')}".strip()
                    ),
                    data=residual,  # type: ignore[arg-type]
                )
            )
        elif residual.get("ok_to_probe"):
            checks.append(
                CheckResult(
                    id="hard_ks_residual",
                    title="Hard kill-switch residual rules",
                    status="pass",
                    detail=str(residual.get("detail") or "No residual hard KS rules."),
                    data=residual,  # type: ignore[arg-type]
                )
            )
        else:
            checks.append(
                CheckResult(
                    ...
                    title="Hard kill-switch residual rules",
                    status="info",
                    detail=str(residual.get("detail") or "Residual probe skipped."),
                    data=residual,  # type: ignore[arg-type]
                )
            )

        if system == "Windows":
            tor_bins = KillSwitch.windows_tor_program_candidates()
            checks.append(
                CheckResult(
                    id="windows_tor_egress_allowlist",
                    title="Windows Tor egress allow-list guidance",
                    status="pass" if tor_bins else "info",
                    detail=(
                        "Tor binary found for hard-KS program allow: "
                        + ", ".join(str(p) for p in tor_bins)
                        if tor_bins
                        else (
                            "No Tor.exe auto-detected. If you use hard kill-switch, "
                            "allow your Tor / Tor Browser program path outbound "
                            "before applying the block (see HARDENING.md)."
                        )
                    ),
                    data={"tor_programs": [str(p) for p in tor_bins]},
                )
            )
