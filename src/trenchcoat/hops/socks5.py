...
class SocksConnectRefused(ConnectionError):
    """SOCKS greeting succeeded; CONNECT to the probe dest was refused."""

    def __init__(self, rep: int) -> None:
        super().__init__(f"SOCKS5 CONNECT refused (rep={rep}); listener is up")
        self.rep = rep
...
class Socks5Hop(Hop):
    async def probe(self, timeout: float = 5.0) -> HopStatus:
        ...
        except SocksConnectRefused as exc:
            latency = (time.perf_counter() - start) * 1000
            self.status = HopStatus(
                hop_id=self.config.id,
    ...
    async def _handshake(self) -> None:
        ...
        try:
            ...
            if hdr[1] != 0:
                # Speaks SOCKS5 but CONNECT to the probe dest failed. Not HEALTHY.
                if hdr[0] == 5:
                    raise SocksConnectRefused(hdr[1])
