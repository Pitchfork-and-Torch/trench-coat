...
class HttpConnectRefused(ConnectionError):
    """HTTP proxy answered; CONNECT to the probe dest was not established."""

    def __init__(self, line: bytes) -> None:
        snippet = line[:80]
        super().__init__(f"HTTP proxy CONNECT not established: {snippet!r}; listener is up")
        self.line = line
...
class HttpProxyHop(Hop):
    async def probe(self, timeout: float = 5.0) -> HopStatus:
        ...
        except HttpConnectRefused as exc:
            latency = (time.perf_counter() - start) * 1000
            self.status = HopStatus(
                hop_id=self.config.id,
