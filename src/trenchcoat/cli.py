...
def up_cmd(
    ...
    from trenchcoat.engine.status_bus import OwnerBusy, claim_owner, owner_alive, release_owner

    alive, owner_pid = owner_alive()
    if alive:
        console.print(
            f"[red]Cloak already owned by pid {owner_pid}. Disengage first (Nexus or stop that process).[/]"
        )
        sys.exit(1)
