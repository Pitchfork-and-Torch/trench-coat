...
@click.option("--json", "as_json", is_flag=True, help="Print hop receipt as JSON (no secrets).")
def receipt_cmd(as_json: bool) -> None:
    """Local hop receipt: chain name, hop types, enabled flags. Never prints passwords."""
    cfg = load_config()
    chain = cfg.get_chain()
    hops = []
    if chain:
    ...
    payload = {
        "kind": "trench-hop-receipt",
        "version": __version__,
        "codename": __codename__,
        "chain": chain.name if chain else None,
        "hop_count": len(hops),
        "enabled_hops": sum(1 for row in hops if row["enabled"]),
        "hops": hops,
    }
    import json
    ...
    if as_json:
        ...
        return
    title = payload["chain"] or "(no chain)"
    console.print(
        f"Hop receipt {payload['version']}  {title}  hops={payload['enabled_hops']}/{payload['hop_count']}"
    )
...
def up_cmd(
    ...
    me = os.getpid()
    blocker = owner_blocks(me)
    if blocker is not None:
        console.print(
            f"[red]Cloak already owned by pid {blocker}. Disengage first (Nexus or `trench down`).[/]"
        )
        sys.exit(1)
    ...
    def _install_stop_signals() -> None:
        # Nexus Disengage sends CTRL_BREAK to CREATE_NEW_PROCESS_GROUP children.
        if os.name != "nt":
            return
        try:
            def _on_break(_signum: int, _frame: object) -> None:
                from trenchcoat.engine.status_bus import request_stop
            ...
            _signal.signal(_signal.SIGBREAK, _on_break)  # type: ignore[attr-defined]
    ...
    async def _run() -> None:
        ...
        claimed = False
        engine: CloakEngine | None = None
        try:
            try:
                claim_owner(me)
                claimed = True
            except OwnerBusy as exc:
                console.print(f"[red]Cloak already owned by pid {exc.pid}.[/]")
                session.add("error", f"owner-busy pid {exc.pid}")
                session.close()
                session.save_json()
                return
            ...
            console.print(
                Panel(
                    ...
                    f"{say('engage')}\n\n"
                    "[dim]Ctrl+C or `trench down` to disengage.[/]",
                    title="CLOAK ENGAGED",
                    border_style="#00FF9F",
                )
            )
...
@main.command("down")
def down_cmd() -> None:
    """Disengage the pid-owned cloak (same path as Nexus Disengage)."""
    from trenchcoat.engine.status_bus import stop_owner

    result = stop_owner()
    mode = str(result.get("mode") or "")
    if mode == "idle":
        console.print("[dim]No cloak process to stop.[/]")
        return
    if mode == "stale":
        console.print(
            f"[yellow]Stale lock cleared (pid {result.get('pid')}). Process was not killed.[/]"
        )
        return
    if not result.get("ok"):
        console.print(f"[red]Disengage failed: {result.get('error') or 'stop failed'}[/]")
        sys.exit(1)
    console.print(f"[#00FF9F]Disengaged[/] pid {result.get('pid')}.")
    console.print(say("disengage"))
