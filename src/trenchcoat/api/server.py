...
def create_app(engine_holder: dict[str, Any] | None = None) -> FastAPI:
    ...
    async def status() -> dict[str, Any]:
        ...
        if bus and bus.get("running"):
            ...
            bus["owner_pid"] = owner_pid
            bus["owner_alive"] = alive
            if not alive:
                # Stale bus must not paint CLOAKED.
                bus["running"] = False
                bus["stale"] = True
                bus["messages"] = [
                    "Stale cloak status: owner pid is dead. Disengage to clear. Do not treat as CLOAKED."
                ]
            return bus
        # Live probe fallback. Port-open alone is not a cloak.
        cfg = load_config()
        entry_up = port_open(cfg.listen_host, cfg.listen_port)
        tor = detect_tor()
        messages: list[str] = []
        if alive and not entry_up:
            messages.append(
                f"Engage pid {owner_pid} is alive; cloak entry not confirmed yet."
            )
        elif entry_up and not alive:
            messages.append(
                "Something is listening on the cloak port, but no pid-owned trench process. "
                "Do not treat as CLOAKED."
            )
        else:
            messages.append("No cloak process status yet - run trench up.")
        return {
            "running": False,
            "version": __version__,
            "chain_name": cfg.active_chain,
            "listen": f"{cfg.listen_host}:{cfg.listen_port}" if entry_up else None,
            "listen_port_open": entry_up,
            ...
            "refuse_direct": True,
            "source": "probe",
            "owner_pid": owner_pid if alive else None,
            "owner_alive": alive,
            "pending": bool(alive and not entry_up),
            "messages": messages,
        }
    ...
    async def cloak_up(body: CloakUpBody) -> dict[str, Any]:
        ...
        try:
            claim_owner(proc.pid, breakaway=True)
        ...
        # Child used to see the pre-claimed pid and abort. Wait long enough to
        # catch immediate fail-closed / crash; do not pretend CLOAKED.
        ...
        while asyncio.get_event_loop().time() < deadline:
            ...
            if code is not None:
                ...
                raise HTTPException(
                    status_code=503,
                    detail=(
                        "Engage process exited before the cloak was up "
                        f"(exit {code}). Fail-closed or crash. Check hops / Tor."
                    ),
                )
            bus = read_status(max_age_seconds=30.0)
            if bus and bus.get("running"):
                return {
                    "ok": True,
                    "mode": "subprocess",
                    ...
                    "running": True,
                    "pending": False,
                    "message": "Cloak engaged. Watch /api/status or Disengage to stop.",
                }
        ...
        if proc.poll() is not None:
            ...
            raise HTTPException(
                status_code=503,
                detail="Engage process exited before the cloak was up. Check hops / Tor.",
            )
        return {
            "ok": True,
            "mode": "subprocess",
            ...
            "pending": True,
            "message": (
                "Engage process live. Waiting for hops or Tor. "
                "Watch status; Disengage to stop. Not CLOAKED yet."
            ),
        }
    ...
    async def cloak_down() -> dict[str, Any]:
        ...
        msg = {
            ...
            "stale": (
                f"Stale lock cleared (pid {result.get('pid')}). "
